...
const char* getNameCASES(enum tagCASES index){
...
#define CASES_COLUMN_DATE(member, number, title)\
	 time_t prozubi_case_get_##number(\
			struct case_t *c)\
	{\
		return c->member;\
	}
#define CASES_COLUMN_TEXT(member, number, title)\
	 char * prozubi_case_get_##number(\
			struct case_t *c)\
	{\
		return c->member;\
	}\
	 size_t prozubi_case_get_len_##number(\
			struct case_t *c)\
	{\
